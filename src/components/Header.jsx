...
<div id='logo'>
<h1 className='text-white font-bold text-5xl '>Barber <span className='italic text-themeyellow '>Shop</span></h1>
</div>
<ul className='lg:flex justify-center items-center gap-6 hidden'>
   {navItems.map(({ link , path })=>(
    <Link key={path} className='text-white uppercase font-semibold cursor-pointer p-3 rounded-lg hover:bg-themeyellow hover:text-black ' to={path} spy={true} offset={-100} smooth={true}> {link}  </Link>
   ))}
</ul>
...
<button className='bg-themeyellow px-8 py-3 rounded-full hover:bg-white hover:text-black font-bold mt-3 hidden lg:flex transform hover:scale-110 transition-transform duration-300 cursor-pointer'>BOOK NOW</button>
 </nav>
 )
